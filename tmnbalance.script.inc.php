...
xecho("tmnbalance v".TMN_VERSION."\n");
if (file_exists(TMN_BALANCE_SEMAPHORE) && (posix_getpgid(intval(file_get_contents(TMN_BALANCE_SEMAPHORE))) !== false) ) {
  xecho("Already running (PID ".sprintf('%d',file_get_contents(TMN_BALANCE_SEMAPHORE)).")\n");
  die(10);
}
file_put_contents(TMN_BALANCE_SEMAPHORE,sprintf('%s',getmypid()));

xecho('Retrieving MN info (mainnet): ');
$result = tmn_cmd_get('/masternodes',array("testnet"=>0),$response);
...
$result = tmn_cmd_get('/balances',array(),$response);
if ($response['http_code'] == 200) {
  echo "Fetched...";
...
foreach($mnpubkeys as $mnpubkey) {
  if (array_key_exists($mnpubkey,$mnlastupdate)) {
    ...
    if ((($argc > 1) && ($argv[1] == 'force')) || ($delta > TMN_BALANCE_INTERVAL)) {
      $mncheck[] = $mnpubkey;
    }
    else {
      $mnchecknot[] = $mnpubkey;
...
foreach($mncheck as $mnpubkey) {
  ...
  $url = str_replace('%%p%%',$mnpubkey,TMN_BALANCE_URL_MAINNET);
...
foreach($tncheck as $mnpubkey) {
  ...
  $url = str_replace('%%p%%',$mnpubkey,TMN_BALANCE_URL_TESTNET);
...
if (count($payload) > 0) {
  ...
  $response = '';
  $content = tmn_cmd_post('/balances',$payload,$response);
