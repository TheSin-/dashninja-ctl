...
//  $version = $config[$testnet]['Version'];
  ...
  try
  {
    $c = new \Terracoin\Node($ip,$port,TMN_PORTCHECK_TIMEOUT,TMN_VERSION,$sversion,$protocol,$magic);
    $subver = $c->getSubVer();
    $c->closeConnection();
    $res = 1;
  }
  catch (\Terracoin\EFailedToReadFromPeer $eftrfp) {
    $subver = '';
    $errmsg = $eftrfp->getMessage();
    $res = 1;
  }
  catch (\Terracoin\EUnexpectedFragmentation $euf) {
    $subver = '';
    $errmsg = $euf->getMessage();
    $res = 1;
  }
  catch (\Terracoin\EUnexpectedPacketType $eupt) {
    $subver = '';
...
xecho("tmnportcheckdo v".TMN_VERSION."\n");
...
$result = tmn_cmd_get('/portcheck/config',array(),$response);
...
if ($argc == 5) {
  ...
  $out = array($ip,$port,$testnet,$mnstatus,$subver,$errormsg,date('Y-m-d H:i:s',time()+TMN_PORTCHECK_INTERVAL));
